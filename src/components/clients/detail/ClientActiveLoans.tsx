...
export async function ClientActiveLoans({ clientId }: ClientActiveLoansProps) {
  const result = await getLoans({ search: clientId, limit: "100" });
    ...
    <div className="mb-6 space-y-6 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 2xl:grid-cols-3 gap-6 items-stretch">
        {loans.map((loan) => (
          <Card
            key={loan.id}
            className="relative flex flex-col overflow-hidden group hover:border-slate-900 dark:hover:border-primary transition-all duration-300 shadow-sm hover:shadow-xl border-zinc-200 dark:border-zinc-800 bg-card"
          >
              ...
              <div className="flex justify-between items-start mb-2">
                <div className="space-y-1">
                  ...
                  <p className="text-sm font-bold text-foreground">
                    ID: {loan.id.slice(0, 12).toUpperCase()}
                  </p>
                </div>
            ...
            <CardContent className="py-6 flex-1 space-y-6">
              <div className="grid grid-cols-2 gap-y-4 gap-x-6">
                ...
                <div className="space-y-1">
                  ...
                  <span className="text-sm font-semibold text-foreground">
                    {(loan.interest_rate * 100).toFixed(2)}% - {loan.term} terms
                  </span>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-zinc-100 dark:border-zinc-800/50 flex align-middle justify-between">
                ...
                <div className="space-y-1 text-right">
                  ...
                  <span className="text-sm font-semibold text-foreground">
                    {new Date(loan.created_at).toLocaleDateString("es-DO", {
                      day: "2-digit",
                      month: "short",
                      year: "numeric",
                    })}
