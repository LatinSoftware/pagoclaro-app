        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground">
                  ID: {client.cedula} • Last Pay: {client.lastPaymentDate ? new Date(client.lastPaymentDate).toLocaleDateString('es-ES', { month: 'short', day: 'numeric' }) : 'Never'}
                </span>
            ...
            <div className="text-right flex flex-col items-end">
              <span className={cn(
                "text-sm font-bold",
                // client.status === 'overdue' ? "text-destructive" : "text-foreground"
              )}>
                ${(client. outstandingBalance ?? 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}
              </span>
              <span className="text-[10px] text-muted-foreground uppercase font-medium">
                {/* {client.status === 'overdue' ? 'Outstanding' : 'Balance'} */}
              </span>
